...
export default function EdgeCacheDemoPage() {
  const [property, setProperty] = useState<Property | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [requestLogs, setRequestLogs] = useState<RequestLog[]>([]);
  const [currentHeaders, setCurrentHeaders] = useState<Record<string, string>>({});
  const [stats, setStats] = useState({ totalRequests: 0, hits: 0, misses: 0, avgTTFB: 0, totalTime: 0 });
  ...
  const fetchProperty = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/CF/api/property/cf-demo-1?strategy=basic&t=${Date.now()}`);
  ...
  const clearCache = async () => {
    try {
      ...
      setStats({ totalRequests: 0, hits: 0, misses: 0, avgTTFB: 0, totalTime: 0 });
      setRequestLogs([]);
      // Yeni istek yap
      fetchProperty();
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-green-50 border-b border-green-200 px-4 py-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-medium text-green-900">Basic Edge Cache - İlk istek MISS, sonrakiler HIT</h3>
            <p className="text-sm text-green-700 mt-1">
              İlk istek origin&apos;e gider ve <strong>CF-Cache-Status: MISS</strong> döner (~800-1200ms). 
              Sonraki istekler edge&apos;den gelir ve <strong>CF-Cache-Status: HIT</strong> döner (~10-25ms). 
              &quot;Cache Temizle&quot; butonuyla cache&apos;i sıfırlayabilirsiniz.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="text-xs text-gray-500 mt-1">{stats.hits} HIT / {stats.misses} MISS</div>
            </div>
            ...
            {/* Cache Age */}
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="text-sm text-gray-600 mb-2">Cache Age</div>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-blue-600">{currentHeaders['age'] || '—'}</span>
                <span className="text-gray-500">saniye</span>
              </div>
            </div>
          </div>
                ...
                <tbody>
                  {requestLogs.length === 0 ? (
                    ...
                    requestLogs.map((log, i) => (
                      <tr key={log.id} className={i === 0 ? (log.cacheStatus === 'HIT' ? 'bg-green-50' : 'bg-yellow-50') : 'bg-white'}>
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 rounded text-xs font-medium ${getCacheStatusColor(log.cacheStatus)}`}>
                            {log.cacheStatus}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3 w-64">
                          <div className="flex items-center gap-2">
                            <div className="flex-1 h-3 bg-gray-200 rounded-full overflow-hidden">
                              <div 
                                className={`h-full transition-all ${log.cacheStatus === 'HIT' ? 'bg-green-500' : 'bg-yellow-500'}`}
              ...
              <div className="p-4 font-mono text-sm">
                {Object.entries(currentHeaders).map(([key, value]) => (
                  <div key={key} className={`py-1 ${
                    key.toLowerCase().startsWith('cf-') || key.toLowerCase() === 'cache-control' || key.toLowerCase() === 'age'
